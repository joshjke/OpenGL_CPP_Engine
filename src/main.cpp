...
const std::vector<GLfloat> primitive_cube()
{
	static const std::vector<GLfloat> g_vertex_buffer_data = {
		-1.0f,-1.0f,-1.0f,
		-1.0f,-1.0f, 1.0f,
...
const std::vector<GLfloat> uvBufferData()
{
...
{
	static const std::vector<GLfloat> g_vertex_buffer_data = primitive_cube();
...
GLuint initializeColorBuffer()
{
	static const std::vector<GLfloat> g_color_buffer_data = uvBufferData();
...
{
	return texture::loadDDS(imagepath);
}
...
void bindAttributeBuffer(GLuint buffer, int attribute, int size)
{
...
void draw(GLFWwindow* window, GLuint programID, GLuint vertexbuffer, GLuint colorbuffer, mat4 mvp, GLuint Texture)
{
...
// Only during the initialisation
	...
	bindAttributeBuffer(vertexbuffer, 0, 3);
	bindAttributeBuffer(colorbuffer, 1, 2);
...
void terminate(GLuint vertexbuffer, GLuint colorbuffer, GLuint programID, GLuint VertexArrayID)
{
...
int main( void )
{
	...
	GLuint VertexArrayID = initializeVertexArray();
	GLuint programID = initializeShaders();
	GLuint vertexbuffer = initializeVertexBuffer();
	GLuint Texture = LoadTexture("uvtemplate.DDS");
	GLuint colorbuffer = initializeColorBuffer();
	mat4 mvp = initializeMVP();
	

	do{
		draw(window, programID, vertexbuffer, colorbuffer, mvp, Texture);
		glfwPollEvents();
		//GameLoop();
	...
	terminate(vertexbuffer, colorbuffer, programID, VertexArrayID);
